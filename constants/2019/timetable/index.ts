...
In this talk we will illustrate the use of WebAssembly and WebGL to create platform agnostic visualization applications for mechanical CAD drawings.`,
    ...
    roomB: {
      speakers: [
        { image: "/static/2019/speakers/zoom_zoomzo.jpg", name: "須藤 欧佑" },
      ],
...
`,
  ...
  },
  {
    ...
    roomB: {
      speakers: [
        { image: "/static/2019/speakers/aimee.JPG", name: "Aimee" },
        {
...
`,
    ...
    roomB: {
      speakers: [
        { image: "/static/2019/speakers/sugino.jpg", name: "杉野 嘉紀" },
      ],
...
ドメイン駆動設計と呼ばれる設計手法を利用し複雑な要件を持つサービスの開発をRustで行っている事例についてご紹介できればと思います。`,
    ...
    roomB: {
      speakers: [
        { image: "/static/2019/speakers/qnighy.jpg", name: "原 将己" },
      ],
      title: "頑固なWebフレームワークを作る",
      abstract:
        "Ruby on Railsのような一部のWebフレームワークは特定のアーキテクチャーを強制することから頑固なフレームワーク (an opinionated framework) と呼ばれています。頑固なフレームワークにはいくつかの利点があります。ディレクトリの配置方法を統一することで、プロジェクトに入りたての人でもコードベースを把握しやすいこと。また、特定のアーキテクチャが自然に適用されるようになっているので、プロジェクトが急成長しても全体の設計が崩れにくいことが期待されます。私はRustをWeb分野でより広く採用できるようにするには、そのような頑固なWebフレームワークがあることが望ましいと考えており、Railsなどのいくつかのフレームワークを参考に試作を進めています。本講演ではその設計や実装にあたって考えたことや得られた知見について話します。",
      language: "日本語",
    },
  },
  {
